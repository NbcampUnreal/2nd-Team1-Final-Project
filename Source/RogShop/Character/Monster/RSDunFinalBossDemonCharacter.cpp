...
#include "RSDunFinalBossDemonCharacter.h"
#include "RogShop/UtilDefine.h"
#include "RogShop/RSMonsterAttackTraceDefine.h"

void ARSDunFinalBossDemonCharacter::PlayBaseAttackAnim()
{
...
void ARSDunFinalBossDemonCharacter::PlayHitReactAnim()
{
	PlayAnimMontage(HitReactMontage);
	UE_LOG(LogTemp, Warning, TEXT("Boss SpiderQueen HitReact Success!!"));
}

void ARSDunFinalBossDemonCharacter::PlayDeathAnim()
{
	PlayAnimMontage(DeathMontage);
	UE_LOG(LogTemp, Warning, TEXT("Boss SpiderQueen Death Success!!"));
}

void ARSDunFinalBossDemonCharacter::PerformAttackTrace()
{
	FHitResult HitResult;
	FVector Start = GetMesh()->GetSocketLocation("WingMiddle2_r"); // 여기다 실제 소켓 이름으로 바꿔줘야 함.
