...
#include "RSDunChickenCharacter.h"
#include "RogShop/UtilDefine.h"
#include "RogShop/RSMonsterAttackTraceDefine.h"

void ARSDunChickenCharacter::PlayBaseAttackAnim()
{
...
void ARSDunChickenCharacter::PlayHitReactAnim()
{
	PlayAnimMontage(HitReactMontage);
	UE_LOG(LogTemp, Warning, TEXT("Boss SpiderQueen HitReact Success!!"));
}

void ARSDunChickenCharacter::PlayDeathAnim()
{
	PlayAnimMontage(DeathMontage);
	UE_LOG(LogTemp, Warning, TEXT("Boss SpiderQueen Death Success!!"));
}

void ARSDunChickenCharacter::PerformAttackTrace()
{
	FHitResult HitResult;
	FVector Start = GetMesh()->GetSocketLocation("beard"); // 여기다 실제 소켓 이름으로 바꿔줘야 함.
